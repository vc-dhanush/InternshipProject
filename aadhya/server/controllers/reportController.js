...
async function classReport(req, res, next) {
  try {
    ...
    for (const st of students) {
      const att = await studentAttendanceSummary(st._id, classId);
      rows.push({
        ...st,
        id: st._id,
        ...att,
        lowAttendance: att.percentage < threshold,
      });
    }
...
async function studentReport(req, res, next) {
  try {
    ...
    const settings = await CollegeSettings.findOne({ user: req.user._id });
    const att = await studentAttendanceSummary(student._id, student.class?._id);
    ...
    res.json({
      ...
      attendance: att,
      history,
      lowAttendance: att.percentage < (settings?.minAttendancePercent ?? 75),
...
async function lowAttendance(req, res, next) {
  try {
    ...
    const flagged = [];
    for (const st of students) {
      const att = await studentAttendanceSummary(st._id, st.class?._id);
...
async function exportMarksCsv(req, res, next) {
  try {
    ...
    const marks = await Mark.find({ test: test._id }).populate("student", "name rollNo studentId").lean();
